...
AUTHOR_NAME="${GITLAB_USER_LOGIN} <${GITLAB_USER_EMAIL}>" #follow CI_COMMIT_AUTHOR format
...
shopt -s extglob #for +($'\n')
[[ "${CI_COMMIT_DESCRIPTION:0:1}" =~ ^$'\n' ]] && CI_COMMIT_DESCRIPTION="${CI_COMMIT_DESCRIPTION:1}" #remove leading newline
...
GCHAT_WEBHOOK_DATA='{
  "name": "'$CI_COMMIT_SHORT_SHA'-'$CI_PIPELINE_IID'", "text": "", "previewText": "", "fallbackText": "fallback", "argumentText": "argument",
        ...
        "imageStyle": "AVATAR", "imageAltText": "GitLab Icon"
      }, "sections": [ { "collapsable": false, "uncollapsableWidgetsCount": 0, "header": "", "widgets": [
            {
              "keyValue": {
                ...
                "content": "Subject: '"$CI_COMMIT_TITLE"'<br>Message:<i>'"${CI_COMMIT_DESCRIPTION//+($'\n')/<br>}"'</i>",
